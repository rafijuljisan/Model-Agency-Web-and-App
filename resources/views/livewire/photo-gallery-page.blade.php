        ...
        <div class="masonry-grid anim-fade-up anim-d1">
            @foreach($photos as $photo)
                <div class="photo-card" 
                     @click="modalOpen = true; modalImage = '{{ Storage::url($photo->image) }}'; modalCaption = '{{ addslashes($photo->caption) }}'">
                    
                    <img src="{{ Storage::url($photo->image) }}" alt="{{ $photo->caption ?? 'Gallery Image' }}" loading="lazy">
