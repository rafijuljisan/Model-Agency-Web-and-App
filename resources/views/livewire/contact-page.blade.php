...
<div class="form-page">
    ...
    <div class="form-grid-2">
        ...
        {{-- LEFT COLUMN: Contact Details Section --}}
            ...
            <div class="form-section-body">
                ...
                @if($settings->contact_address)
                    <div class="contact-block">
                        ...
                        <div class="contact-text whitespace-pre-line">{{ $settings->contact_address }}</div>
                    </div>
                @endif
                ...
                <div class="contact-block">
                    ...
                    @if($settings->contact_email)
                        <div class="contact-text mb-1"><a href="mailto:{{ $settings->contact_email }}">{{ $settings->contact_email }}</a></div>
                    @endif
                    @if($settings->contact_phone)
                        <div class="contact-text"><a href="tel:{{ $settings->contact_phone }}">{{ $settings->contact_phone }}</a></div>
                    @endif
                </div>
                ...
                @if($settings->business_hours)
                    <div class="contact-block">
                        ...
                        <div class="contact-text">{{ $settings->business_hours }}</div>
                    </div>
                @endif
                ...
                <div class="contact-block mt-8">
                    ...
                    <div class="contact-socials">
                        @if($settings->facebook_url)
                            <a href="{{ $settings->facebook_url }}" target="_blank" rel="noopener noreferrer" class="contact-social-btn" aria-label="Facebook">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>
                            </a>
                        @endif
                        @if($settings->instagram_url)
                            <a href="{{ $settings->instagram_url }}" target="_blank" rel="noopener noreferrer" class="contact-social-btn" aria-label="Instagram">
                        ...
                        @if($settings->youtube_url)
                            <a href="{{ $settings->youtube_url }}" target="_blank" rel="noopener noreferrer" class="contact-social-btn" aria-label="YouTube">
                        ...
                        @if($settings->linkedin_url)
                            <a href="{{ $settings->linkedin_url }}" target="_blank" rel="noopener noreferrer" class="contact-social-btn" aria-label="LinkedIn">
        ...
        <div class="form-section anim-fade-up anim-d2">
            ...
            {{-- We remove padding here to let the map flush perfectly to the edges --}}
